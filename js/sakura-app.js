...
function imgError(ele, type) {
    switch (type) {
        case 1:
            ...
            break;
        case 2:
            ele.src = 'https://g.misakamoe.com/avatar/?s=80&d=mm&r=g';
            break;
        default:
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.status == 200) {
                        ...
                        $('#upload-img-show').append('<img class="lazyload upload-image-preview" src="https://jsdelivr.misakamoe.com/gh/moezx/cdn@3.0.2/img/svg/loader/trans.ajax-spinner-preloader.svg" data-src="' + get_the_url + '" onclick="window.open(\'' + get_the_url + '\')" onerror="imgError(this)" />');
                        lazyload();
...
function checkDarkModeCookie() {
    var dark = getCookie("dark"),
        today = new Date(),
        hour = today.getHours();
        if (mashiro_option.darkmode && ((!dark && (hour > 18 || hour < 7) ) || (dark == '1' && (hour >= 19 || hour <= 6)))) {
            setTimeout(function () {
                $("#dark-bg").click();
            }, 100);
...
function coverVideoIni() {
    if ($('video').hasClass('hls')) {
        if (mashiro_global.variables.has_hls){
            ...
            $.getScript("https://jsdelivr.misakamoe.com/gh/mashirozx/Sakura@3.3.3/cdn/js/src/16.hls.js", function(){
                loadHls();
...
function motionSwitch(ele) {
    var motionEles = [".bili", ".menhera", ".hoyo", ".ba"];
...
loadCSS("https://jsdelivr.misakamoe.com/npm/aplayer@1.10.1/dist/APlayer.min.css");
            ...
            _btn.on('click', function () {
                if ($(this).hasClass('loadvideo')) {
                    ...
                    if ($(this).hasClass('video-pause')) {
                        ...
                        $('.headertop-down').removeAttr("style","");
                    } else {
                        Siren.splay();
...
function get_poem(poem_ele, info_ele) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('get', 'https://v2.jinrishici.com/one.json?client=browser-sdk/1.2&X-User-Token=lZP4R9DXWahzgScosAFsLPSPNBqx6Qis');
...
function loadBotui() {
    if ($('div').hasClass('popcontainer')) {
        if (mashiro_global.variables.has_bot_ui) {
            ...
            $.getScript('https://jsdelivr.misakamoe.com/gh/zxy678/Misakaloli/botui.js',
